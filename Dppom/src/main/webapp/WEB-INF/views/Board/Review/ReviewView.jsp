			...
			<div class="row row-bottom-padded-lg" id="about-us">
				<div class="col-md-12 to-animate">
					<div class="text-center">
						...
						<!-- 테이블 S -->
						<form action="" name="viewfrm">
							<input type="hidden" name="filename"
								value="<%=request.getParameter("filename") %>" />
							<% String board_type = request.getParameter("filename"); %>
							<table id="commu_table" class="table  table-bordered">
								...
								<tbody>
									...
									<tr>
										<td>${viewRow.univ }</td>
										<td>${viewRow.major }</td>
										<td style="width: 30%"><c:forEach begin="1"
												end="${viewRow.attachedfile/2 }" var="i">
                              ★
                           </c:forEach> <%-- 평점 ${viewRow.attachedfile } 점  --%></td>
									</tr>
									<tr>
								...
								<c:when test="${viewRow.id == sessionScope.login_id.id }">
									...
									<div class="col-md-12" style="text-align: center">
										<button type="button" class="btn"
											onclick="javascript:location.href='ModifyReview?idx=${viewRow.idx}&filename=<%=request.getParameter("filename") %>'"
											style="margin-bottom: 5px;">수 정</button>
										<button type="button" class="btn"
											onclick="javascript:location.href='deleteView?id=${viewRow.id}&filename=<%=request.getParameter("filename") %>&id2=${sessionScope.login_id.id }&idx=${viewRow.idx}'"
								...
								<tr>
									...
									<td>
										<button class="btn btn-xs btn-danger" type="button"
											onclick="replyReport('${sessionScope.login_id.id}','${re.idx}','review');">신고</button>
									</td>
									<td>${re.regidate}</td>
									<td>
										<div class="row">
