...
<script>
	function userPointCheck(point) {
		if (point < 50) {
			alert("회원의 포인트가 50점 미만이므로 파일을 다운로드 받을 수 없습니다.");
			return false;
		}
	}
	...
	function boardreport(id, board_idx, board_type, board_id) { // 신고id , 글번호 , 게시판 type , 글제목 , 작성자 id 
		...
		if (isReport == true) {
			...
			if (report_reason == null) {
				...
				location.href = 'boardReportAction?id=' + id + '&board_idx='
						+ board_idx + '&report_reason=' + report_reason
	...
	function replyReport(id, reply_idx, return_view) { // 신고id , 글번호 , 게시판 type , 글제목 , 작성자 id 
		...
		if (isReport == true) {
			...
			if (report_reason == null) {
				...
				location.href = 'replyReportAction?id=' + id + '&reply_idx='
						+ reply_idx + '&report_reason=' + report_reason
									...
									<tr>
										<th colspan="3" style="width: 10%; text-align: right;">
											<button type="button" class="btn btn-danger"
												...
												style="height: 40px; margin-bottom: 4px">신고</button> <a
											href="likeup?id=${viewRow.id }&idx=<%=request.getParameter("idx") %>&filename=<%=request.getParameter("filename") %>&nowPage=<%=request.getParameter("nowPage") %>&category=<%=request.getParameter("category") %>"
											name="좋아요" style="color: red"> <img
												src="./resources/bootstrap3.3.7/images/좋아요.png" alt="" />
										</a><span style="font-size: 2em;">${viewRow.like_cnt }</span>
										</th>
									</tr>
								</thead>
								<tbody>
									...
									<tr>
										...
										<td style="width: 30%">
											<button class="btn"
												onclick="return userPointCheck('${point}');">${viewRow.attachedfile}</button>
										</td>
									</tr>
									<tr>
								...
								<c:when test="${viewRow.id == sessionScope.login_id.id }">
									...
									<div class="col-md-12" style="text-align: center">
										<button type="button" class="btn"
											onclick="javascript:location.href='ModifyData?idx=${viewRow.idx}&board_type=${viewRow.board_type }&nowPage=<%=request.getParameter("nowPage") %>&category=${viewRow.category }&filename=<%=request.getParameter("filename") %>'"
											style="margin-bottom: 5px;">수 정</button>
										<button type="button" class="btn"
											onclick="javascript:location.href='deleteData?id=${viewRow.id}&id2=${sessionScope.login_id.id }&idx=${viewRow.idx}&nowPage=<%=request.getParameter("nowPage") %>'"
								...
								<tr>
									...
									<td>
										<button class="btn btn-xs btn-danger" type="button"
											onclick="replyReport('${sessionScope.login_id.id}','${re.idx}','dataroom');">신고</button>
									</td>
									<td>${re.regidate}</td>
									<td>
										<div class="row">
