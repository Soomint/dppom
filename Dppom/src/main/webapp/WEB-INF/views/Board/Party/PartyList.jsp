<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ page session="true"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<%@ include file="/WEB-INF/layout/head.jsp"%>
<style>
.navbar-brand {
	color: #6173f4;
}

#cate:hover {
	background-color: black;
	color: white !important;
}
</style>
<title>大PPOM - Party</title>
<link rel="shortcut icon" href="./resources/bootstrap3.3.7/images/D.ico" />

<script>
   function checkserchFrm(){
      var check = document.searchFrm;
      // 폼 객체 엘리먼트를 불러옴
      
      if(check.sel1.value=="선택"){
         alert("선택해주세요.");
         check.sel1.focus();
         return false;
      }
      
      if(check.search.value==""){
         alert("검색할 내용을 입력하세요.");
         check.search.focus();
         return false;
      }
   }
   
   function checkFrm1(){
       alert("로그인을 해주세요.");
       location.href='Login?';
       return false;
 }
 function checkFrm2(){
        alert("권한이 없습니다. 학생인증을 해주세요.");
        return false;
      }
    
 function checkWriteFrm1(){
       alert("로그인을 해주세요");
       location.href='Login?';
       return false;
 }
 function checkWriteFrm2(){
        alert("권한이 없습니다. 학생인증을 해주세요.");
        return false;
      }
</script>
</head>
<body>
	<header role="banner" id="fh5co-header"> <!--  TOP ( 메뉴 )  -->
	<%@ include file="/WEB-INF/layout/LayoutTop_black.jsp"%>
	<%@ include file="/WEB-INF/layout/LayoutTop1_black.jsp"%>
	<!-- TOP (E) --> </header>

	<div id="fh5co-about-us" data-section="about">
		<div class="container">
			<div class="row" id="team">
				<div class="col-md-12 section-heading text-center to-animate">
					<h2>PARTY</h2>
				</div>
			</div>
			<div class="row row-bottom-padded-lg" id="about-us">
				<div class="col-md-12 section-heading text-center to-animate">
					<div class="row">
						<c:choose>
							<c:when test="${category=='팀프로젝트'}">
								<div class="col-md-4">
									<h3>
										<a id="" href="Party?PageURL=Party&category=팀프로젝트"
											style="text-decoration: none; color: #6c727a;"> <b><img
												src="./resources/bootstrap3.3.7/images/팀플_클릭.png" alt="팀플" /></b></a>
									</h3>
								</div>
								<div class="col-md-4">
									<h3>
										<a id="" href="Party?PageURL=Party&category=스터디"
											style="text-decoration: none; color: #6c727a;"> <img
											src="./resources/bootstrap3.3.7/images/스터디.png" style=""
											alt="" /></a>
									</h3>
								</div>
								<div class="col-md-4">
									<h3>
										<a id="" href="Party?PageURL=Party&category=etc"
											style="text-decoration: none; color: #6c727a;"> <img
											src="./resources/bootstrap3.3.7/images/ETC.png" alt="" /></a>
									</h3>
								</div>
							</c:when>
							<c:otherwise>
								<c:if test="${category=='스터디'}">
									<div class="col-md-4">
										<h3>
											<a id="" href="Party?PageURL=Party&category=팀프로젝트"
												style="text-decoration: none; color: #6c727a;"> <img
												src="./resources/bootstrap3.3.7/images/팀플.png" alt="" /></a>
										</h3>
									</div>
									<div class="col-md-4">
										<h3>
											<a id="" href="Party?PageURL=Party&category=스터디"
												style="text-decoration: none; color: #6c727a;"> <b>
													<img src="./resources/bootstrap3.3.7/images/스터디_클릭.png"
													alt="" />
											</b></a>
										</h3>
									</div>
									<div class="col-md-4">
										<h3>
											<a id="" href="Party?PageURL=Party&category=etc"
												style="text-decoration: none; color: #6c727a;"> <img
												src="./resources/bootstrap3.3.7/images/ETC.png" alt="" /></a>
										</h3>
									</div>
								</c:if>
								<c:if test="${category=='etc'}">
									<div class="col-md-4">
										<h3>
											<a id="" href="Party?PageURL=Party&category=팀프로젝트"
												style="text-decoration: none; color: #6c727a;"> <img
												src="./resources/bootstrap3.3.7/images/팀플.png" alt="" /></a>
										</h3>
									</div>
									<div class="col-md-4">
										<h3>
											<a id="" href="Party?PageURL=Party&category=스터디"
												style="text-decoration: none; color: #6c727a;"> <img
												src="./resources/bootstrap3.3.7/images/스터디.png" alt="" /></a>
										</h3>
									</div>
									<div class="col-md-4">
										<h3>
											<a id="" href="Party?PageURL=Party&category=etc"
												style="text-decoration: none; color: #6c727a;"> <b><img
													src="./resources/bootstrap3.3.7/images/ETC_클릭.png" alt="" /></b></a>
										</h3>
									</div>
								</c:if>
							</c:otherwise>
						</c:choose>
					</div>
				</div>
				<div class="col-md-12 to-animate">
					<div class="text-center">
						<div class="row">
							<!-- 글쓰기 버튼 -->
							<div class="col-md-12" style="text-align: right">
								<c:if test="${sessionScope.login_id.id eq null }">
									<form action="PartyWrite" onsubmit="return checkWriteFrm1();">
										<button type="submit" class="btn" style="margin-bottom: 5px;">Write</button>
									</form>
								</c:if>
								<c:if test="${sessionScope.login_id.certi_flag eq 'X'}">
									<form action="PartyWritev" onsubmit="return checkWriteFrm2();">
										<button type="submit" class="btn" style="margin-bottom: 5px;">Write</button>
									</form>
								</c:if>
								<c:if test="${sessionScope.login_id.certi_flag eq 'O'}">
									<form action="PartyWrite">
										<button type="submit" class="btn" style="margin-bottom: 5px;">Write</button>
									</form>
								</c:if>
							</div>
						</div>
						<!-- 테이블 S -->
						<table id="commu_table" class="table table-hover table-bordered">
							<thead>
								<tr>
									<th style="width: 10%; text-align: center;">No</th>
									<th style="width: 10%; text-align: center;">종류</th>
									<th style="text-align: center;">제목</th>
									<th style="width: 10%; text-align: center;">작성자</th>
									<th style="width: 10%; text-align: center;">참가인원</th>
									<th style="width: 10%; text-align: center;">날짜</th>
									<th style="width: 10%; text-align: center;">조회수</th>
								</tr>
							</thead>
							<tbody>
								<c:choose>
									<c:when test="${empty listRows}">
										<!-- 등록된글이 없는경우 노출 -->
										<tr>
											<td colspan="7" style="height: 100px;">등록된 글이 없습니다.</td>
										</tr>
									</c:when>
									<c:otherwise>
										<c:forEach items="${listRows}" var="row" varStatus="loop">
											<!-- 글이 있는경우 노출 -->
											<tr>
												<td style="text-align: center;">${row.rNum }</td>
												<td style="text-align: center;">${row.category }</td>
												<c:if test="${sessionScope.login_id.id eq null }">
													<td><a href="" onclick="return checkFrm1();">${row.title}</a></td>
												</c:if>
												<c:if test="${sessionScope.login_id.certi_flag eq 'X' }">
													<td><a href="" onclick="return checkFrm2();">${row.title}</a></td>
												</c:if>
												<c:if test="${sessionScope.login_id.certi_flag eq 'O' }">
													<td><a
														href="./PartyView?idx=${row.idx }&nowPage=${nowPage}&category=${row.category}&filename=${filename}"
														style="color: Black">${row.title}</a></td>
												</c:if>
												<c:if test="${row.id == null }">
													<td style="text-align: center;">탈퇴회원</td>
												</c:if>
												<c:if test="${row.id != null }">
													<td style="text-align: center;">${row.id }</td>
												</c:if>
												<c:if test="${row.now_party == row.total_party}">
													<td style="text-align: center;"><img
														src="./resources/bootstrap3.3.7/images/모집완료수정.png"
														alt="모집완료" /></td>
												</c:if>
												<c:if test="${row.now_party != row.total_party}">
													<td style="text-align: center;">${row.now_party}/
														${row.total_party }</td>
												</c:if>
												<td style="text-align: center;">${row.regidate }</td>
												<td style="text-align: center;">${row.click_cnt }</td>
											</tr>
										</c:forEach>
									</c:otherwise>
								</c:choose>
							</tbody>
							<%--    <%} //for문 끝%>
               <%}//else문 끝 %>    --%>
						</table>
						<!-- 테이블 E  -->
						<!-- 검색 S -->
						<form name="searchFrm" action="<c:url value="searchPartyAction"/>"
							onsubmit="return checkserchFrm();">
							<div class="row">
								<div class="col-md-2">
									<select name="sel1" class="form-control" id="sel1">
										<option>선택</option>
										<option value="title">제목</option>
										<option value="id">ID</option>
										<option value="category">종류</option>
									</select>
								</div>
								<div class="col-md-6 input-group">
									<span class="input-group-addon"><i
										class="glyphicon glyphicon-search"></i></span> <input name="search"
										style="width: 70%" id="email" type="text" class="form-control"
										name="search"> <input type="hidden" name="board_type"
										value="${board_type }" /> <input type="hidden"
										name="category" value="${category }" /> <input type="hidden"
										name="filename" value="Party" />
									<button type="submit" class="btn btn-default"
										style="margin-left: 0px" onclick="">Search</button>
								</div>
							</div>
						</form>
						<!-- 검색 E -->
						<br />
						<!-- 페이징 -->
						<table width="100%">
							<tr>
								<td style="text-align: center;">${pagingImg}</td>
							</tr>
						</table>
						<br />
					</div>
				</div>
			</div>
		</div>
	</div>
	<!-- Botton (S) -->
	<%@ include file="/WEB-INF/layout/LayoutBottom.jsp"%>
	<!--  Bottom(E) -->
</body>
</html>