			...
			<div class="row row-bottom-padded-lg" id="about-us">
				...
				<div class="col-md-12 to-animate">
					<div class="text-center">
						<div class="row">
							<form action="mypointPlus" method="get">
								...
								<div class="col-md-12" style="text-align: right">
									...
									<select
										...
										<option
											value="myHome?id=${sessionScope.login_id.id }&nowPage=1">&nbsp;&nbsp;전체</option>
										<option
											value="mypointPlus?id=${sessionScope.login_id.id }&used_type=적립&nowPage=1"
											style="color: blue"
											<c:if test="${used_type =='적립' }"> selected </c:if>>&nbsp;&nbsp;적립</option>
										<option
											value="mypointPlus?id=${sessionScope.login_id.id }&used_type=차감&nowPage=1"
											style="color: red"
											<c:if test="${used_type =='차감' }"> selected </c:if>>&nbsp;&nbsp;차감</option>
									</select>
								</div>
							</form>
						</div>
						...
						<table id="commu_table" class="table table-hover table-bordered">
							...
							<tbody>
								<c:choose>
									<c:when test="${empty mySearchPoint }">
										<!-- 등록된글이 없는 경우  -->
										<tr>
									...
									<c:otherwise>
										<c:forEach items="${mySearchPoint }" var="list"
											varStatus="loop">
											<!-- 글이 있는 경우  -->
											<tr>
