...
<script>
	function chkFrm() {
		if (document.getElementById("dropoutAgree").checked != true) {
			alert('약관 동의해 주세요');
			return false;
		}
	}
						...
						<div id="container">
							...
							<div id="content">
								...
								<div class="section_delete">
									<h3>
										사용하고 계신 아이디(<em>${sessionScope.login_id.id }</em>)는 탈퇴할 경우 재사용
										및 복구가 불가능합니다.
									</h3>
									<p>
									...
									<p>
										회원정보 등 개인형 서비스 이용기록은 모두 삭제되며, 삭제된 데이터는 복구되지 않습니다.<br>삭제되는
										내용을 확인하시고 필요한 데이터는 미리 백업을 해주세요.
									</p>
									탈퇴 후 회원정보 및 개인형 서비스 이용기록은 모두 삭제됩니다.
									<h3>탈퇴 후에도 게시판형 서비스에 등록한 게시물은 그대로 남아 있습니다.</h3>
									<p>
										올린 게시글 및 댓글은 탈퇴 시 자동 삭제되지 않고 그대로 남아 있습니다. <br>삭제를 원하는
										게시글이 있다면 <em>반드시 탈퇴 전 비공개 처리하거나 삭제하시기 바랍니다.</em> <br>탈퇴
										후에는 회원정보가 삭제되어 본인 여부를 확인할 수 있는 방법이 없어, 게시글을 임의로 삭제해드릴 수 없습니다.
										탈퇴 후에도 게시판형 서비스에 등록한 게시물은 그대로 남아 있습니다.
									</p>
									<form name="fm" id="fm" method="post" action="">
										<input type="hidden" name="token_help"
											value="EiXDb6AeIEsWm7X3" /> <input type="hidden" name="lang"
											value="ko_KR" />
										<div>
											<p>
												<strong>탈퇴 후에 아이디와 데이터는 복구할 수 없습니다. <br> 게시판형
													서비스에 남아 있는 게시글은 탈퇴 후 삭제할 수 없습니다.
												</strong>
											</p>
											<input type="checkbox" id="dropoutAgree" name="dropoutAgree"
												onclick=""> <label for="dropoutAgree"><strong>안내
													사항을 모두 확인하였으며, 이에 동의합니다.</strong></label>
										</div>
										<div>
											<p>
												<a href="myOut2?id=${sessionScope.login_id.id }" id=""
													class="" onclick="return chkFrm()"><b>확인</b></a>
											</p>
										</div>
									</form>
								</div>
